	...
	const char* d_amino_startpos, char* d_pop, float* d_objval, const int len_amino_seq, const int cds_num, const int cycle, const float mprob, const int limit, const float lowest_mcai)
{
	...
	//if (threadIdx.x == 0)
	//{
	//	printf("block id : %d   idx : %d    %d\n",blockIdx.x, ptr_origin_objidx[_MLRCS * 2], ptr_origin_objidx[_MLRCS * 2 + 1]);
	//	printf("blcok id : %d    p : %d    q : %d    l :   %d\n", blockIdx.x,ptr_origin_lrcsval[P], ptr_origin_lrcsval[Q], ptr_origin_lrcsval[L]);
	//}
