...
int main()
{
    ...
    global_cycle = 1;
    // int tmp_cycle[] = {5350,5950,5000,4900,4550,3900};
    int test_cycle[] = {100, 100, 100, 100, 100};
    ...
    // for (int zz = 0; zz < 1; zz++)
    {
        ...
        //  {
        pop_size = 128;
        // total_cycle = tmp_cycle[zz];
        for (int s1 = 0; s1 < 1; s1++)
        {
            total_cycle = test_cycle[s1];
            ...
            for (int z = 0; z < global_cycle; z++)
            {
                ...
                free(h_pop);
                free(h_objval);
                printf("protein : \n using shared memory : %d \t %d",sizeof(int) * (threadsPerBlock + 3) + sizeof(float) * (threadsPerBlock + OBJECTIVE_NUM) + sizeof(char) * (len_sol + len_amino_seq + 2 * OBJECTIVE_NUM), sizeof(int) * (threadsPerBlock + 3 * 2) + sizeof(float) * (threadsPerBlock + OBJECTIVE_NUM * 2) + sizeof(char) * (len_sol * 2 + len_amino_seq + OBJECTIVE_NUM * 2 * 2 + 1));
