...
char *tmp_pop, float* tmp_objval, char * tmp_objidx, int *tmp_lrcsval)
{
    ...
    tmp_objidx[threadIdx.x * OBJECTIVE_NUM *2 + 2*_mCAI] = d_objidx[threadIdx.x * OBJECTIVE_NUM *2 +2*_mCAI];
    tmp_objidx[threadIdx.x * OBJECTIVE_NUM *2 +2*_mHD] = d_objidx[threadIdx.x * OBJECTIVE_NUM *2 +2*_mHD];
    tmp_objidx[threadIdx.x * OBJECTIVE_NUM *2 +2*_mHD + 1] = d_objidx[threadIdx.x * OBJECTIVE_NUM *2 +2*_mHD + 1];
    tmp_objidx[threadIdx.x * OBJECTIVE_NUM *2 +2*_MLRCS] = d_objidx[threadIdx.x * OBJECTIVE_NUM *2 +2*_MLRCS];
    tmp_objidx[threadIdx.x * OBJECTIVE_NUM *2 +2*_MLRCS + 1] = d_objidx[threadIdx.x * OBJECTIVE_NUM *2 +2*_MLRCS + 1];
    ...
    d_objidx[threadIdx.x * OBJECTIVE_NUM *2 + 2*_mCAI] = tmp_objidx[d_sorted_array[threadIdx.x] * OBJECTIVE_NUM *2 +2*_mCAI];
    d_objidx[threadIdx.x * OBJECTIVE_NUM *2 +2*_mHD] = tmp_objidx[d_sorted_array[threadIdx.x] * OBJECTIVE_NUM *2 +2*_mHD];
    d_objidx[threadIdx.x * OBJECTIVE_NUM *2 +2*_mHD + 1] = tmp_objidx[d_sorted_array[threadIdx.x] * OBJECTIVE_NUM *2 +2*_mHD + 1];
    d_objidx[threadIdx.x * OBJECTIVE_NUM *2 +2*_MLRCS] = tmp_objidx[d_sorted_array[threadIdx.x] * OBJECTIVE_NUM *2 +2*_MLRCS];
    d_objidx[threadIdx.x * OBJECTIVE_NUM *2 +2*_MLRCS + 1] = tmp_objidx[d_sorted_array[threadIdx.x] * OBJECTIVE_NUM *2 +2*_MLRCS + 1];
	...
	//d_sorted_array[threadIdx.x] = blockDim .x / 2 - 1;
...
int main()
{
	...
	h_objidx = (char*)malloc(sizeof(char) * twice_pop * OBJECTIVE_NUM * 2);
	h_lrcsval = (int*)malloc(sizeof(int) * twice_pop * 3);
	h_sorted_array = (int*)malloc(sizeof(int) * twice_pop);
	...
	CHECK_CUDA(cudaEventRecord(d_start))
		for (i = 0; i < k; i++)
		{
			...
			// CHECK_CUDA(cudaMemcpy(h_objidx, d_objidx, sizeof(char) * twice_pop * OBJECTIVE_NUM * 2, cudaMemcpyDeviceToHost))
			//	sorting
			FastSortSolution << <1, twice_pop, sizeof(Sol) * twice_pop + sizeof(int) * twice_pop * 2 + sizeof(float) * twice_pop * OBJECTIVE_NUM>> > 
				(d_sorted_array, d_F_set, d_Sp_set, d_np, d_rank_count, d_sol_struct, d_pop,d_objval,d_objidx,d_lrcsval,tmp_pop,tmp_objval,tmp_objidx,tmp_lrcsval);
			// CHECK_CUDA(cudaMemcpy(h_sorted_array, d_sorted_array, sizeof(int) * twice_pop, cudaMemcpyDeviceToHost))
			// for(j=0;j<pop_size;j++)
			// {
			// 	fprintf(tmp_fp, "%f %f %f\n", -h_objval[h_sorted_array[j] * OBJECTIVE_NUM + _mCAI], -h_objval[h_sorted_array[j] * OBJECTIVE_NUM + _mHD], h_objval[h_sorted_array[j] * OBJECTIVE_NUM + _MLRCS]);
			// }
			// fprintf(tmp_fp,"\n");
			//CHECK_CUDA(cudaMemcpy(h_objval, d_objval, sizeof(float) * twice_pop * OBJECTIVE_NUM, cudaMemcpyDeviceToHost))
			//CHECK_CUDA(cudaMemcpy(h_sorted_array, d_sorted_array, sizeof(float) * pop_size, cudaMemcpyDeviceToHost))

				//for (j = 0; j < pop_size; j++)
				//	printf("%d\n", h_sorted_array[j]);
				//for (j = 0; j < pop_size * 2; j++)
				//{
				//	printf("%d : %f %f %f\n", j + 1, h_objval[j * OBJECTIVE_NUM + _mCAI], h_objval[j * OBJECTIVE_NUM + _mHD], h_objval[j * OBJECTIVE_NUM + _MLRCS]);
				//}
		}
	//fclose(tmp_fp);
	CHECK_CUDA(cudaEventRecord(d_end))
	...
	CHECK_CUDA(cudaMemcpy(h_pop, d_pop, sizeof(char) * twice_pop * len_sol, cudaMemcpyDeviceToHost))
		CHECK_CUDA(cudaMemcpy(h_objval, d_objval, sizeof(float) * twice_pop * OBJECTIVE_NUM, cudaMemcpyDeviceToHost))
		CHECK_CUDA(cudaMemcpy(h_objidx, d_objidx, sizeof(char) * twice_pop * OBJECTIVE_NUM * 2, cudaMemcpyDeviceToHost))
		CHECK_CUDA(cudaMemcpy(h_lrcsval, d_lrcsval, sizeof(int) * twice_pop * 3, cudaMemcpyDeviceToHost))
		CHECK_CUDA(cudaMemcpy(h_sorted_array, d_sorted_array, sizeof(int) * twice_pop, cudaMemcpyDeviceToHost))
