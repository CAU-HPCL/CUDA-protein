...
__host__ __device__ bool compCAI(const Sol& a, const Sol& b) {
	return a.obj_val[_mCAI] < b.obj_val[_mCAI];
}

__host__ __device__ bool compHD(const Sol& a, const Sol& b) {
	return a.obj_val[_mHD] < b.obj_val[_mHD];
}

__host__ __device__ bool compMLRCS(const Sol& a, const Sol& b) {
	return a.obj_val[_MLRCS] < b.obj_val[_MLRCS];
}

__host__ __device__ bool compDist(const Sol& a, const Sol& b) {
	return a.corwding_dist < b.corwding_dist;
}
...
__global__ void FastSortSolution(int* d_sorted_array, const float* d_objval, bool* F_set, bool* Sp_set, int* np, int* rank_count, Sol* sol_struct)
{
	...
	if (count > (c_sort_popsize / 2))
	{
		...
		{
			if(i == 0)	
				thrust::sort(sol_struct, sol_struct + rank_count[front], compCAI);
			else if (i == 1)
				thrust::sort(sol_struct, sol_struct + rank_count[front], compHD);
			else if (i == 2)
				thrust::sort(sol_struct, sol_struct + rank_count[front], compMLRCS);
		...
		thrust::sort(sol_struct, sol_struct + rank_count[front], compDist);
		grid.sync();
