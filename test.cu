...
int main()
{
    ...
    for (i = 0; i < j; i++)
    {
        ...
        CHECK_CUDA(cudaLaunchCooperativeKernel((void **)FastSortSolution, (twice_pop + deviceProp.maxThreadsPerBlock - 1)/deviceProp.maxThreadsPerBlock, twice_pop / ((twice_pop + deviceProp.maxThreadsPerBlock - 1)/deviceProp.maxThreadsPerBlock) + 1, args))
    }
    CHECK_CUDA(cudaEventRecord(d_end))
    CHECK_CUDA(cudaEventSynchronize(d_end))
    CHECK_CUDA(cudaEventElapsedTime(&kernel_time, d_start, d_end))
