...
__constant__ int c_cutted_popsize;
__constant__ float c_percent;
...
__global__ void FastSortSolution(int *d_sorted_array, bool *F_set, bool *Sp_set, float *d_objval, Sol *d_sol_struct, int *d_rank_count, int *d_np)
{
    ...
    if (count > (c_sort_popsize / 2))
    {
        ...
        // if first non-dominated size > (c_sort_popsize /2)  half doubled
        // 여기는 rank 에서 확률만큼 잘라서 하는 곳
        if (g.thread_rank() < (c_sort_popsize / 2))
        {
            d_sorted_array[g.thread_rank()] = d_sol_struct[g.thread_rank() % (int)(d_rank_count[0] * c_percent)].sol_idx;
        }
        // printf("% \n",g.thread_rank() % (int)(d_rank_count[0] * c_percent));
...
int main()
{
    ...
    const char *ff_result[] = {
        "/home/jeus5771/CUDA_protein/NSGA2_sorting_half/NSGA2_sorting_half_rank/NSGA2_sorting_half_rank_reuslt1.csv",
        "/home/jeus5771/CUDA_protein/NSGA2_sorting_half/NSGA2_sorting_half_rank/NSGA2_sorting_half_rank_reuslt2.csv",
        "/home/jeus5771/CUDA_protein/NSGA2_sorting_half/NSGA2_sorting_half_rank/NSGA2_sorting_half_rank_reuslt3.csv",
        "/home/jeus5771/CUDA_protein/NSGA2_sorting_half/NSGA2_sorting_half_rank/NSGA2_sorting_half_rank_reuslt4.csv",
        "/home/jeus5771/CUDA_protein/NSGA2_sorting_half/NSGA2_sorting_half_rank/NSGA2_sorting_half_rank_reuslt5.csv",
        "/home/jeus5771/CUDA_protein/NSGA2_sorting_half/NSGA2_sorting_half_rank/NSGA2_sorting_half_rank_reuslt6.csv"};
    ...
    scanf("%d", &x);
    // printf("Input number of global cycle : ");
    // scanf("%d", &global_cycle);
    //  printf("For sorting kernel use number of twice_pop threads per block");
    printf("Input global cycle : ");
    scanf("%d", &global_cycle);
    float percent;
    int cutted_popsize;
    printf("Input percenct 0 <  < 1 : ");
    scanf("%f", &percent);
    printf("\nFor sorting kernel use number of twice_pop threads per block");
    ...
    for (int zz = 0; zz < sizeof(ff_cds_num) / sizeof(int); zz++)
    {
        ...
        {
            pop_size = 128;
            cutted_popsize = (int)(pop_size * percent);
            CHECK_CUDA(cudaMemcpyToSymbol(c_cutted_popsize, &cutted_popsize, sizeof(int)))
            CHECK_CUDA(cudaMemcpyToSymbol(c_percent, &percent, sizeof(float)))
            for (int s1 = 0; s1 < 5; s1++)
            {
                fprintf(global_fp, "\ncycle : %d, pop_size : %d\n", total_cycle, pop_size);
                for (int z = 0; z < global_cycle; z++)
                {
                    ...
                    int numCoopBlock = (twice_pop + deviceProp.maxThreadsPerBlock - 1) / deviceProp.maxThreadsPerBlock;
                    int numCoopThreads = (twice_pop % numCoopBlock == 0) ? (twice_pop / numCoopBlock) : (twice_pop / numCoopBlock) + 1;

                    void *args[] = {&d_sorted_array, &d_F_set, &d_Sp_set, &d_objval, &d_sol_struct, &d_rank_count, &d_np};
                    j = (total_cycle % sorting_cycle == 0) ? total_cycle / sorting_cycle : total_cycle / sorting_cycle + 1;
                    CHECK_CUDA(cudaEventRecord(d_start))
                    // to check >>> rank
                    int *h_rank_count = (int *)malloc(sizeof(int) * twice_pop);
                    for (i = 0; i < j; i++)
                    {
                        ...
                        // for check
                        CHECK_CUDA(cudaMemcpy(h_rank_count, d_rank_count, sizeof(int) * twice_pop, cudaMemcpyDeviceToHost))
                        if (h_rank_count[0] >= pop_size)
                            fprintf(global_fp, "%d,", i + 1);
                    }
                    fprintf(global_fp, "\n");
                    CHECK_CUDA(cudaEventRecord(d_end))
                    CHECK_CUDA(cudaEventSynchronize(d_end))
