...
__constant__ int c_len_cds;
__constant__ int c_len_sol;
__constant__ float c_mprob;
__constant__ int c_pop_size;
__constant__ int c_twice_pop_size;
...
__global__ void Kerenl(curandStateXORWOW *state, const char *d_amino_seq_idx, char *d_pop, float *d_objval, char *d_objidx, int *d_lrcsval, const int cycle,
					   int *d_sorted_array, bool *F_set, bool *Sp_set, int *np, int *rank_count, Sol *sol_struct)
{
	...
	int i, j, k, l;
	int idx, seq_idx;
	int b_partition, t_partition;
	int lrcs_p, lrcs_q, lrcs_l, tmp_l;

	/* -------------------- shared memory allocation -------------------- */
	extern __shared__ int smem[];
	__shared__ int *s_lrcs_tid;
	__shared__ int *s_origin_sol_lrcsval;
	__shared__ int *s_target_sol_lrcsval;
	__shared__ int * s_b_idx;
	__shared__ float *s_obj_compute; // for computing mCAI & mHD value
	__shared__ float *s_origin_sol_objval;
	__shared__ float *s_target_sol_objval;
	__shared__ char *s_amino_seq_idx;
	__shared__ char *s_origin_sol;
	__shared__ char *s_target_sol;
	__shared__ char *s_origin_sol_objidx;
	__shared__ char *s_target_sol_objidx;
	__shared__ char *mutation_type;

	s_lrcs_tid = smem;
	s_origin_sol_lrcsval = (int *)&s_lrcs_tid[blockDim.x]; // for finding which thread have LRCS
	s_target_sol_lrcsval = (int *)&s_origin_sol_lrcsval[3];
	s_b_idx = (int *)&s_target_sol_lrcsval[3];
	s_obj_compute = (float *)&s_b_idx[1];
	s_origin_sol_objval = (float *)&s_obj_compute[blockDim.x];
	s_target_sol_objval = (float *)&s_origin_sol_objval[OBJECTIVE_NUM];
	s_amino_seq_idx = (char *)&s_target_sol_objval[OBJECTIVE_NUM];
	s_origin_sol = (char *)&s_amino_seq_idx[c_len_amino_seq];
	s_target_sol = (char *)&s_origin_sol[c_len_sol];
	s_origin_sol_objidx = (char *)&s_target_sol[c_len_sol];
	s_target_sol_objidx = (char *)&s_origin_sol_objidx[OBJECTIVE_NUM * 2];
	mutation_type = (char *)&s_target_sol_objidx[OBJECTIVE_NUM * 2];
	/* -------------------- end of shared memory allocation -------------------- */

	localState = state[g.thread_rank()];

	t_partition = (c_len_amino_seq % tb.size() == 0) ? (c_len_amino_seq / tb.size()) : (c_len_amino_seq / tb.size() + 1);
	for (i = 0; i < t_partition; i++)
	{
	...
	/* Initialize solutions in population */
	s_b_idx
	for (i = 0; i < c_pop_size; i++)
	{
		if(tb.thread_rank() == 0)
			
		if (g.blo == gridDim.x - 1)
		{
	...
	/* Read population from global memory based on d_sorted_arrya */
...
int main()
{
	...
	for (i = 0; i < k; i++)
	{
		...
		{
			mainKernel<<<numBlocks, threadsPerBlock, sizeof(int) * (threadsPerBlock + 3 * 2) + sizeof(float) * (threadsPerBlock + OBJECTIVE_NUM * 2) + sizeof(char) * (len_sol * 2 + len_amino_seq + OBJECTIVE_NUM * 2 * 2 + 1)>>>(genState, d_amino_seq_idx, d_pop, d_objval, d_objidx, d_lrcsval, total_cycle % sorting_cycle, d_sorted_array);
		}
		else
		{
			mainKernel<<<numBlocks, threadsPerBlock, sizeof(int) * (threadsPerBlock + 3 * 2) + sizeof(float) * (threadsPerBlock + OBJECTIVE_NUM * 2) + sizeof(char) * (len_sol * 2 + len_amino_seq + OBJECTIVE_NUM * 2 * 2 + 1)>>>(genState, d_amino_seq_idx, d_pop, d_objval, d_objidx, d_lrcsval, sorting_cycle, d_sorted_array);
		}
		//	sorting
