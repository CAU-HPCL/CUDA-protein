...
__global__ void mainKernel(curandStateXORWOW *state, const char *d_amino_seq_idx, char *d_pop, float *d_objval, char *d_objidx, int *d_lrcsval, const int cycle, const int limit, const float lowest_mcai)
{
			...
			if (ptr_origin_objval[_mCAI] < section_low)
			{
				...
				for (i = 0; i < num_partition; i++)
				{
					...
					if (idx < c_len_amino_seq * c_cds_num)
					{
						...
						pos = FindNum_C(&c_codons[c_amino_startpos[s_amino_seq_idx[seq_idx]] * CODON_SIZE], &ptr_origin_sol[idx * CODON_SIZE],
										c_codons_num[s_amino_seq_idx[seq_idx]]);
						mutation(&localState, &c_codons[c_amino_startpos[s_amino_seq_idx[seq_idx]] * CODON_SIZE], &ptr_origin_sol[idx * CODON_SIZE],
...
int main()
{
	...
	const char *ff_result[] = {
		"/home/jeus5771/CUDA_protein/pre_proc/pre_proc_result/pre_proc_reuslt1.csv",
		"/home/jeus5771/CUDA_protein/pre_proc/pre_proc_result/pre_proc_reuslt2.csv",
		"/home/jeus5771/CUDA_protein/pre_proc/pre_proc_result/pre_proc_reuslt3.csv",
		"/home/jeus5771/CUDA_protein/pre_proc/pre_proc_result/pre_proc_reuslt4.csv",
		"/home/jeus5771/CUDA_protein/pre_proc/pre_proc_result/pre_proc_reuslt5.csv",
		"/home/jeus5771/CUDA_protein/pre_proc/pre_proc_result/pre_proc_reuslt6.csv"};
	...
	scanf("%d", &cycle); // if number of cycle is zero we can check initial population
	if (cycle < 0)
	{
	...
	for (int zz = 0; zz < sizeof(ff_cds_num) / sizeof(int); zz++)
	{
		...
		for (int z = 0; z < global_cycle; z++)
		{
			...
			mainKernel<<<numBlocks, threadsPerBlock,
						 sizeof(int) * (threadsPerBlock + 3 * 2) + sizeof(float) * (threadsPerBlock + OBJECTIVE_NUM * 2) + sizeof(char) * (len_sol * 2 + len_amino_seq + OBJECTIVE_NUM * 2 * 2 + 1)>>>(genState, d_amino_seq_idx, d_pop, d_objval, d_objidx, d_lrcsval, cycle, limit, lowest_mcai);
			cudaEventRecord(d_end);
			cudaEventSynchronize(d_end);
			cudaEventElapsedTime(&kernel_time, d_start, d_end);
